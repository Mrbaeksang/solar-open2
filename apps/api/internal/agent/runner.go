...
답변은 한국어로 간결하게 쓴다. 숫자형 신뢰도나 확률을 제시하지 않는다.`
	if track == domain.TrackEasy {
		return common + `
대상은 초등 고학년부터 중학생이다. 짧고 구체적인 문장으로 설명한다.
첫 문장에 답을 말하고, 생활 예시, 실제 원리, 비유가 깨지는 지점 순서로 설명한다.
유치한 말투를 쓰지 않는다. 건강·안전·돈·괴롭힘 문제는 신뢰할 수 있는 어른의 도움을 권한다.`
	}
	return common + `
대상은 고등학생 이상 비전공자다. 전문용어는 필요한 경우에만 쓰고 바로 뜻을 설명한다.
핵심 답, 작동 원리, 구체적 예시, 한계 또는 반례, 근거 순서로 답한다.
...
func trustedAdultAnswer(track domain.Track) string {
	if track == domain.TrackEasy {
		return "이 문제는 AI 설명 도우미가 대신 판단할 수 없습니다. 지금 안전이 걱정되거나 약·돈·괴롭힘과 관련됐다면, 혼자 해결하지 말고 보호자·교사처럼 신뢰할 수 있는 어른에게 바로 알려 주세요."
	}
