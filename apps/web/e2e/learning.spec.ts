...
test(
  "reader chooses a track, reads evidence, and answers a check question",
  async ({ page }, testInfo) => {
    await page.goto("/");
    await Promise.all([
    ...
    const citationChip = page
      .getByRole("button", { name: /근거 · OECD 자세히 보기/ })
      .first();
    if (testInfo.project.name === "desktop") {
      await citationChip.hover();
      await expect(
        citationChip.locator("xpath=..").getByRole("tooltip"),
      ).toContainText("출처 미리보기");
    }
    ...
    await expect(
      sourceDialog.getByText(/OECD/).first(),
    ).toBeVisible();
...
test("assistant receives selected reading context through the Go AG-UI endpoint", async ({
  ...
  const sectionText = page
    .locator(".lesson-section p")
    .filter({ hasText: "AI 시스템은 사람이 정한 목표를 위해" })
    .first();
  ...
  await expect(
    page.locator(".chat-answer .assistant-citation-chip").first(),
  ).toBeVisible();
  await expect(page.locator(".chat-answer")).not.toContainText("[1]");
...
test("mobile exposes navigation and assistant as touch-sized controls", async ({
  ...
  const assistantButton = page.getByRole("button", { name: "AI에 질문하기" });
  ...
  await expect(
    page.getByRole("complementary", { name: "기본 트랙 AI 설명 도우미" }),
  ).toBeVisible();
});
